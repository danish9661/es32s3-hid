...
<p align="center">
  <img src="https://img.shields.io/badge/Hardware-ESP32--S3%20N16R8-red?style=for-the-badge&logo=espressif" alt="ESP32-S3 N16R8">
  <img src="https://img.shields.io/badge/USB-Triple%20Interface%20(MSC%20+%20HID%20+%20CCID)-blue?style=for-the-badge&logo=usb" alt="USB Triple Interface">
  <img src="https://img.shields.io/badge/Security-FIDO2%20%7C%20CTAP%202.1%20%7C%20WebAuthn-green?style=for-the-badge&logo=fidoalliance" alt="FIDO2 WebAuthn">
  <img src="https://img.shields.io/badge/Yubico-YubiKey%205A%20Emulation%20(v5.4.3)-purple?style=for-the-badge&logo=yubico" alt="YubiKey 5A">
  <img src="https://img.shields.io/badge/KVM-Ultra--Low%20Latency%20UDP-orange?style=for-the-badge" alt="KVM Bridge">
</p>
...
The **ESP32-S3 HID Console & Security Hub** is a multi-role USB hardware security key, injection platform, and ultra-low latency KVM bridge designed for the **ESP32-S3-DevKitC-1-N16R8** (16 MB Quad-SPI Flash, 8 MB Octal PSRAM).

It unifies three major disciplines into a single pocket-sized microcontroller:
...
3. **Professional BadUSB & Low-Latency Web KVM Bridge** with 2 MB USB Virtual Storage (`DUCKY_DRIVE`), Ducky Script 2.0 parser, and cross-platform mouse/keyboard streaming client (`esp32_kvm.py`).
...
| **HMAC Secret / PRF** | ❌ None | ⚠️ Rare | ✅ **HMAC-Secret Extension** (1Password / Bitwarden offline vault encryption) |
...
| **Zero-Knowledge Backup** | ❌ None | ❌ None | ✅ **AES-256-GCM Encrypted `.esp32vault` export/import** + Dual NVS backup |

---

## 🎯 The 3 Operating Modes

The device switches between 3 distinct profiles either via the Web Dashboard or by **holding the physical `BOOT` button (GPIO 0) for 2.5 seconds** (reboots automatically in 500ms):

```
                               ┌─────────────────────────────┐
                               │  Hold BOOT Button for 2.5s  │
                               └──────────────┬──────────────┘
                                              ▼
                ┌───────────────────────────────────────────────────────────┐
                │             Automatic Hardware Reboot (500ms)             │
                └──────┬──────────────────────┬──────────────────────┬──────┘
                       │                      │                      │
                       ▼                      ▼                      ▼
             🟢 Normal Ducky HID    🔷 Standard FIDO2       🟣 YubiKey 5A Mode
             (Logitech / MSC Drive)  (FIDO Alliance VID)    (Yubico VID: 0x1050)
```
...
### 1. Official YubiKey Manager (`ykman 5.9.2`) CLI Verification
```bash
$ ykman list
...
## 🖥️ Web KVM & BadUSB Automation Engine
...
* 🛡️ [**FIDO2 & CTAP2 Technical Specification**](docs/FIDO2_PASSKEY_SPECIFICATION.md) — Low-level packet structures, cryptographic primitives, and state machines.
...
* ⚙️ [**Settings & Hardware Reboot Guide**](docs/SETTINGS_REBOOT_GUIDE.md) — Real-time vs reboot configuration reference.

---

<p align="center">
  <sub>Built with ❤️ for ESP32-S3 • Compliant with W3C WebAuthn, FIDO Alliance CTAP 2.1 & ISO 7816-4</sub>
</p>
