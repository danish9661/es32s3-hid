...
├──────────────────────────────┬──────────────────────────────┤
│ 🛡️ Cryptographic Engine      │ 💾 Storage & Persistence     │
├──────────────────────────────┼──────────────────────────────┤
...
| :--- | :---: | :--- | :--- |
| **🛡️ Dedicated Passkey Mode** | 🟦 **Neon Cyan** | Standalone FIDO2 Security Key only (Keyboard/Mouse disabled for anti-BadUSB security isolation) | Passkey logins on Google, GitHub, Android, Windows |
...
2. **Web Dashboard**:
   ...
   * Click **`🛡️ Switch to Passkey Mode`** or **`💻 Switch to Normal Ducky Mode`**.
...
1. Ensure the device is in **🛡️ Dedicated Passkey Mode** (or switch to it).
...
Inside the Vault tab, every registered passkey includes an **`👁️ Details`** button. Clicking it opens the deep inspection modal:
...
1. Click **`⬆️ Upload & Restore`** and select your `.esp32vault` file.
...
* Ensure you are in **🛡️ Dedicated Passkey Mode** (Neon Cyan LED).
