...
void USBHIDFIDO::begin(bool dedicatedMode) {
  ...
  if (!workerTaskHandle) {
    xTaskCreatePinnedToCore(fidoTaskWrapper, "FidoWorker", 8192, this, 1, &workerTaskHandle, 1);
  }
  hid.begin();
}
...
void USBHIDFIDO::handleCbor(uint32_t cid, const uint8_t *data, size_t len) {
  ...
  if (ctap2Cmd == 0x0A) { // authenticatorCredentialManagement
    handleCredentialManagement(cid, data, len);
    return;
  }
...
void USBHIDFIDO::handleClientPin(uint32_t cid, const uint8_t *data, size_t len) {
  if (len < 2) {
    sendCborResponse(cid, CTAP2_ERR_INVALID_PARAMETER, nullptr, 0);
    return;
  }
  ...
  // 1. Extract protocol & subCmd from top-level map
  if (cborLen >= 5 && (cbor[0] & 0xE0) == 0xA0 && cbor[1] == 0x01 && cbor[3] == 0x02) {
    protocol = cbor[2];
    subCmd = cbor[4];
  }

  // 2. Extract keyAgreement public key (-2: X, -3: Y) and binary parameters
  for (size_t i = 0; i < cborLen; i++) {
    ...
    } else if (i + 18 <= cborLen && cbor[i] == 0x04 && cbor[i+1] == 0x50) { // pinAuth (16 bytes)
      pinAuth.assign(cbor + i + 2, cbor + i + 18);
    } else if (i + 67 <= cborLen && cbor[i] == 0x05 && cbor[i+1] == 0x58 && cbor[i+2] == 0x40) { // newPinEnc (64 bytes)
      newPinEnc.assign(cbor + i + 3, cbor + i + 67);
    } else if (i + 18 <= cborLen && cbor[i] == 0x06 && cbor[i+1] == 0x50) { // pinHashEnc / currentPinHashEnc (16 bytes)
      if (subCmd == 0x04) {
  ...
  Serial.printf("[PIN] subCmd: 0x%02X, proto: %u, pubX: %u, pubY: %u, pinAuth: %u, newPin: %u, pinHash: %u, curPinHash: %u\n",
    subCmd, protocol, (unsigned)peerPubX.size(), (unsigned)peerPubY.size(), (unsigned)pinAuth.size(), (unsigned)newPinEnc.size(), (unsigned)pinHashEnc.size(), (unsigned)currentPinHashEnc.size());

  if (protocol != 1) {
    sendCborResponse(cid, CTAP2_ERR_PIN_AUTH_INVALID, nullptr, 0);
    return;
  }
  ...
  if (subCmd == 0x02) { // getKeyAgreement
    if (!FidoStore::generateKeyPair(ecdhPrivKey, ecdhPubX, ecdhPubY)) {
      sendCborResponse(cid, CTAP2_ERR_OPERATION_DENIED, nullptr, 0);
      return;
    }
    CborWriter resp;
    resp.writeMap(1);
    resp.writeInt(0x01); // 01: keyAgreement
    resp.writeMap(5);
    resp.writeInt(1);  resp.writeInt(2);  // kty: 2 (EC2)
    resp.writeInt(3);  resp.writeInt(-7); // alg: -7 (ES256)
    resp.writeInt(-1); resp.writeInt(1);  // crv: 1 (P-256)
    resp.writeInt(-2); resp.writeBytes(ecdhPubX.data(), 32); // x
    resp.writeInt(-3); resp.writeBytes(ecdhPubY.data(), 32); // y
    sendCborResponse(cid, CTAP2_OK, resp.buf.data(), resp.buf.size());
    return;
  }
  ...
  if (subCmd == 0x03) { // setPIN
    ...
    if (pinLen < 4) {
      sendCborResponse(cid, CTAP2_ERR_PIN_POLICY_VIOLATION, nullptr, 0);
      return;
    }
    FidoStore::setPin(decryptedPin.data(), pinLen);
    sendCborResponse(cid, CTAP2_OK, nullptr, 0);
    return;
  }

  if (subCmd == 0x04) { // changePIN
    if (!FidoStore::isPinSet()) {
      sendCborResponse(cid, CTAP2_ERR_PIN_NOT_SET, nullptr, 0);
      return;
    }
    ...
    if (!FidoStore::verifyPinHash(decCurrentPinHash.data())) {
      sendCborResponse(cid, CTAP2_ERR_PIN_INVALID, nullptr, 0);
      return;
    }
  ...
  if (subCmd == 0x05 || subCmd == 0x09) { // getPinToken
    ...
    CborWriter resp;
    resp.writeMap(1);
    resp.writeInt(0x02); // 02: pinUvAuthToken
...
void USBHIDFIDO::handleLargeBlob(uint32_t cid, const uint8_t *data, size_t len) {
  if (len < 2) {
    sendCborResponse(cid, CTAP2_ERR_INVALID_PARAMETER, nullptr, 0);
    return;
  }
  ...
  if (isGet) {
    ...
    CborWriter resp;
    resp.writeMap(1);
    resp.writeInt(0x01); // 01: configData
...
void USBHIDFIDO::handleCredentialManagement(uint32_t cid, const uint8_t *data, size_t len) {
  if (len < 2) {
    sendCborResponse(cid, CTAP2_ERR_INVALID_PARAMETER, nullptr, 0);
    return;
  }
  ...
  // Key 1 is ALWAYS the subCmd in credential_mgmt
  ...
  if (subCmd == 0x02) { // enumerateRPsBegin
    if (allCreds.empty()) {
      sendCborResponse(cid, CTAP2_ERR_NO_CREDENTIALS, nullptr, 0);
      return;
    }
